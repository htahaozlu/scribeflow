...
The maintainer checklist for cutting a new ScribeFlow release. The version lives in a
...
The version is single-sourced in `src/scribeflow/__about__.py` (Hatchling reads it
via `[tool.hatch.version]`). Edit one line:

```python
# src/scribeflow/__about__.py
__version__ = "X.Y.Z"
```

Nothing else needs changing — the CLI (`scribeflow --version`) and the package
...
```bash
python -m venv /tmp/scribeflow-rel && /tmp/scribeflow-rel/bin/pip install dist/*.whl
/tmp/scribeflow-rel/bin/scribeflow --version   # should print: scribeflow X.Y.Z
/tmp/scribeflow-rel/bin/scribeflow doctor
```
...
```bash
git add src/scribeflow/__about__.py CHANGELOG.md
...
- PyPI shows the new version: <https://pypi.org/project/scribeflow/>.
- Smoke-test the published package from a clean environment:

  ```bash
  python -m venv /tmp/scribeflow-verify && /tmp/scribeflow-verify/bin/pip install scribeflow
  /tmp/scribeflow-verify/bin/scribeflow --version   # scribeflow X.Y.Z
  ```
...
```bash
# 1. bump src/scribeflow/__about__.py  → __version__ = "X.Y.Z"
# 2. update CHANGELOG.md          → new [X.Y.Z] - YYYY-MM-DD section
# 3. local dry run
python -m build && twine check dist/*
# 4. commit
git add src/scribeflow/__about__.py CHANGELOG.md && git commit -m "release: vX.Y.Z" && git push
# 5. tag
git tag vX.Y.Z && git push origin vX.Y.Z
# 6. publish the GitHub Release (triggers PyPI upload)
gh release create vX.Y.Z --title "vX.Y.Z" --notes-file -
# 7. verify Actions are green + https://pypi.org/project/scribeflow/
```
