...
names are registered so ``scribeflow models`` / ``scribeflow doctor`` can report them and
...
from scribeflow.engine.types import TranscriptionBackend
...
def is_available(name: str) -> bool:
    ...
    if name == "whispercpp":
        ...
        from scribeflow.backends.whispercpp import find_whispercpp_binary
...
def create_backend(
    ...
    **kwargs: Any,
    ...
    if name == "whispercpp":
        from scribeflow.backends.whispercpp import WhisperCppBackend

        return WhisperCppBackend(model, device=device, compute_type=compute_type, **kwargs)
    if name == "openai-whisper":
        from scribeflow.backends.openai_whisper import OpenaiWhisperBackend
