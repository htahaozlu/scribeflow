...
def find_whispercpp_binary(explicit: str | None = None) -> str | None:
    """Locate the whisper.cpp CLI: explicit arg → ``SCRIBEFLOW_WHISPERCPP_BIN`` → PATH."""
    if explicit and Path(explicit).expanduser().exists():
        return str(Path(explicit).expanduser())
    env = os.environ.get("SCRIBEFLOW_WHISPERCPP_BIN")
...
def resolve_model_path(model: str, models_dir: str | None = None) -> Path | None:
    ...
    ``models_dir`` / ``SCRIBEFLOW_WHISPERCPP_MODELS``."""
    ...
    env = os.environ.get("SCRIBEFLOW_WHISPERCPP_MODELS")
    if env:
        ...
        *,
        ...
        if self._binary is None:
            raise RuntimeError(
                ...
                "'whisper-cli' on PATH, or set SCRIBEFLOW_WHISPERCPP_BIN. "
                "(pip install 'scribeflow[cpp]' for the pywhispercpp binding.)"
            )
        ...
        if self._model_path is None:
            raise RuntimeError(
                ...
                "ggml .bin or set SCRIBEFLOW_WHISPERCPP_MODELS to a directory containing "
                f"ggml-{model}.bin."
            )
        self._device = device
