"""Command-line entry point for ScribeFlow (docs/ai/05 §8).
...
from scribeflow.__about__ import APP_NAME, SLUG, __version__
...
from scribeflow.engine.chunking import ensure_ffmpeg, ffmpeg_version
from scribeflow.engine.exporters import export
from scribeflow.engine.pipeline import EngineConfig, run_batch
from scribeflow.engine.types import ChunkingSpec, RuntimeDirs, SourceSpec, TranscribeOptions
from scribeflow.model_policy import MODEL_CATALOG, ModelResolution, auto_select
from scribeflow.notebook.generator import NotebookSpec, generate_colab_notebook
...
MESSAGES: dict[str, dict[str, str]] = {
    ...
    "backend_unavailable": {
        "en": "Backend '{backend}' is unavailable. Install it: pip install 'scribeflow[{extra}]'",
        "tr": "'{backend}' arka ucu yok. Kur: pip install 'scribeflow[{extra}]'",
    },
    "ffmpeg_missing": {
...
def _resolve(cfg: ScribeFlowConfig) -> ModelResolution:
    return auto_select(
...
def _engine_config(cfg: ScribeFlowConfig, dirs: RuntimeDirs) -> EngineConfig:
    return EngineConfig(
...
def cmd_doctor(args: argparse.Namespace) -> int:
    ...
    for name, spec in registry.KNOWN_BACKENDS.items():
        ...
        if available:
            ...
            detail = f"missing (pip install 'scribeflow[{spec.extra}]')"
        else:
            detail = "missing"
...
def cmd_web(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        print(
            style.red("web UI needs the [web] extra: pip install 'scribeflow[web]'"),
            file=sys.stderr,
        )
        return 3
    app = create_app(
...
def build_parser() -> argparse.ArgumentParser:
    ...
    tr.add_argument("--config", default=None, help="path to a scribeflow.toml")
    _add_common(tr)
    ...
    gn.add_argument("-o", "--output", default="scribeflow_colab.ipynb", help="output .ipynb path")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if getattr(args, "ui_lang", None) is None:
        env_lang = os.environ.get("SCRIBEFLOW_LANG") or os.environ.get("LANG", "")
