...
class NotebookSpec:
    source: str
    output_dir: str = "/content/drive/MyDrive/yazit-output"  # durable (Drive)
    workspace_dir: str = "/content/yazit-workspace"  # local scratch (Errno-107 split)
    model: str | None = None  # None → auto-select on the Colab host
    backend: str | None = None
    language: str = "tr"
    chunk_minutes: int = 20
    extras: tuple[str, ...] = ()  # e.g. ("url",) so the notebook installs yazit[url]


def pip_target(extras: tuple[str, ...]) -> str:
    return f"yazit[{','.join(extras)}]" if extras else "yazit"
