...
class ColabRuntime:
    ...
    def resolve_dirs(
        ...
        return RuntimeDirs(
            ...
            workspace_dir=Path(workspace_dir) if workspace_dir else CONTENT / "yazit-workspace",
            # durable — small transcript/checkpoint writes are safe over Drive
            output_dir=Path(output_dir) if output_dir else DRIVE_BASE / "yazit-output",
            cache_dir=Path(cache_dir) if cache_dir else CONTENT / "yazit-cache",
        )
